...
rnd_str : str
    ...
    randomString = prefix.join(random.choice(legal_chars) for _ in range(length))
    return randomString
...
verbose : bool
    ...
    """
    # Insert user code here
    dirdepth = random.randrange(2,rec_depth+1)
    dirPrDir = random.randrange(2,dirs+1)

    #for x in range (3):
    makeDirs(target,dirdepth,dirPrDir)

def makeDirs(dir,dirdeep,dirs):
    rand_s = random_string()
    rand_dirs = random.randrange(1,dirs)

    if dirdeep>=0 :
        for x in range (1,random.randint(1,dirs)):  
            newdir = dir+"/"+random_string()
            os.makedirs(newdir)
            makeDirs(newdir,dirdeep-1,dirs)
            populate_tree(newdir,random.randint(1,dirs),800,1388534400,1406851201000,False)
        dirdeep-=1
